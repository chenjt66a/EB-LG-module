...
def parse_arguments():
    ...
    parser.add_argument('--model', type=str, default='pointnet2_cls_ssg', metavar='N',
                        choices=['pointnet2_cls_ssg','model_cls'],
                        help='Model to use, [pointnet, dgcnn]')
    parser.add_argument('--model_name', type=str, default='DgcnnEBLG', metavar='N',
                        choices=['AdaptEBLG', 'DgcnnEBLG'],
                        help='Model to use')
...
def train(args, io):
    ...
    if args.dataset == 'modelnet40':
        if args.model == 'pointnet2_cls_ssg':
            model = MODEL.get_model(40, normal_channel=False).to(device)
        elif args.model == 'model_cls':
            if args.model_name == 'AdaptEBLG':
                model = MODEL.AdaptEBLG(args, output_channels=40).to(device)
            elif args.model_name == 'DgcnnEBLG':
                model = MODEL.DgcnnEBLG(args, output_channels=40).to(device)
    elif args.dataset == 'ScanObjectNN':
        if args.model == 'pointnet2_cls_ssg':
            model = MODEL.get_model(15, normal_channel=False).to(device)
        elif args.model == 'model_cls':
            if args.model_name == 'AdaptEBLG':
                model = MODEL.AdaptEBLG(args, output_channels=15).to(device)
            elif args.model_name == 'DgcnnEBLG':
                model = MODEL.DgcnnEBLG(args, output_channels=15).to(device)
    print(str(model))
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc > best_test_acc:
            ...
            torch.save(model.state_dict(), 'models/%s/models/model.l27_scanobjnn01_train' % args.name)
            io.cprint('Current best saved in: {}'.format('********** models/%s/models/model.l27_scanobjnn01_train **********' % args.name))


def test(args, io):
    MODEL = import_module(args.model)
    device = torch.device('cpu' if args.gpu_idx[0] < 0 else 'cuda:{}'.format(args.gpu_idx[0]))
    torch.manual_seed(args.seed)
    torch.cuda.manual_seed(args.seed)
    ...
    if len(args.gpu_idx) > 1:
        model = nn.DataParallel(model, device_ids=args.gpu_idx)
    model.load_state_dict(torch.load('models/%s/models/pretrained_models/model.pn2_modelnet' % args.name))
    model = model.eval()
    test_acc = 0.0
    count = 0.0
    test_true = []
